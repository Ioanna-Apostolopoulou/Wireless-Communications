...
/* Global Stuct for ath9k_beacon _config arguments*/
typedef struct args{
	struct ath_softc *sc_arg;
	struct ieee80211_vif *vif_arg;
	bool beacon_args;
}args_t;

args_t time_calculator;
...
int beacon_altering(void *arg){

	while(1){
		do{
			
		}while(!arg);
		ath9k_beacon_config(((args_t*)arg)->sc_arg,
					((args_t*)arg)->vif_arg,
					((args_t*)arg)->beacon_args);
		msleep(6000);
	}
	...
	if(iter_data.beacons == false){
		time_calculator.sc_arg = sc ;
		time_calculator.vif_arg = iter_data.primary_beacon_vif ;
		time_calculator.beacon_args = iter_data.beacons ;
		
		kthread_run(beacon_altering, &time_calculator, "Time_Interval_Calculation");
	}
	ath9k_hw_set_interrupts(ah);
